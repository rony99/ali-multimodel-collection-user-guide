...
- 正式对比模型固定为平台三套：
  ...
  - `glm-5.2`
  - `qwen-3.7-max`
- 须按平台下发的地址 / 密钥配置 Claude Code 连接。
- 禁止用自备 API、本地模型或其它云模型**冒充**这三套成绩。

**文档**：`用户操作步骤.md` 第 1 步；`README.md` FAQ #1。
...
| 关键词 | 条目 |
| --- | --- |
| 自己的模型 / 本地模型 / 自备 API | A1 |
| 通过 / test.sh / eval_pass | C1 |
| 一条数据 / 一条样本 | B1 |
| pass@4 / 60% / 难度 | B4, C2 |
| multi-sessions / 只交包 | D1, D2, D3 |
| 不同题面 / instruction | E2 |
| 预检 / 审核 | C4, G1, G3 |
| 轮次 / turns / 20 / 100 | C2, F1 |
| 多题 / 批次 / 50% | C3 |
| Baseline / GT / 偷看答案 | B3, E1 |
| solve.sh | F2 |
| agents / call-level | B2, D5 |
