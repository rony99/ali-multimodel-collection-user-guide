...
| **[qa_skill/](./qa_skill/)** | **答疑 Skill**：加载后按口径回答用户常见问题 |
| **[上传前预检/](./上传前预检/)** | 上传前结构预检 Skill（预审 ≠ 终审） |

建议顺序：**用户操作步骤** → 对照样例 → 上传前预检；有疑问可先加载 [qa_skill](./qa_skill/)；字段/门槛吃不准时再翻甲方要求说明。
...
| # | 问题 | 一句话答案 |
| --- | --- | --- |
| 1 | [是否可以使用自己的模型？](#1-是否可以使用自己的模型) | **不可以**（仅平台三套） |
| 2 | [怎么才算「通过」？](#2-怎么才算通过) | **只看** `test.sh` 退出码 `0` |
| 3 | [「一条数据」是一次模型跑分吗？](#3-一条数据是一次模型跑分吗) | **不是**；一条 = 一道完整题 |
| 4 | [只交甲方数据包可以吗？](#4-只交甲方数据包可以吗) | **不可以**；须另交平级 `multi-sessions/` |
| 5 | [三模型可以用不同题面吗？](#5-三模型可以用不同题面吗) | **不可以**；同一份 `instruction.md` |
| 6 | [预检通过算审核通过吗？](#6-预检通过算审核通过吗) | **不算**；终审以实现网+甲方为准 |
| 7 | [Opus 交几次 session？](#7-opus-交几次-session) | 至少 **≥2**，建议 **4**（合格仍按 pass@4≤60%） |

更全口径见 [qa_skill/faq.md](./qa_skill/faq.md)；让 Agent 答疑时加载 [qa_skill/SKILL.md](./qa_skill/SKILL.md)。

---

### 1. 是否可以使用自己的模型？

<span style="color:#d93025">不可以。</span>
...
<span style="color:#d93025">不算。</span> 上传前预检只查结构与完整性；通过率 / 区分度须自测；**以实现网和甲方的后期审核为准**。

### 7. Opus 交几次 session？

<span style="color:#d93025">`multi-sessions/` 里 Opus 至少 ≥2 次</span>独立完整 `.jsonl`（建议交齐 **4** 次对齐 pass@4）。  
正式题目合格仍按 Opus **pass@4 ≤ 60%**（同题独立 4 次至多过 2 次）。
